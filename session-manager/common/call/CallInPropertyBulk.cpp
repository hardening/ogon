...
static wLog *logger_property = WLog_Get("ogon.sessionmanager.call.property");
...
namespace ogon { namespace sessionmanager { namespace call {
	...
	bool CallInPropertyBulk::encodeResponse() {
		...
		for (RequestedProps::const_iterator it = mProps.begin(); it != mProps.end(); ++it) {
			...
			if (it->success) {
				...
				case ::ogon::icp::PROP_STRING:
					...
					break;
				default:
					WLog_Print(logger_property, WLOG_ERROR, "unknown property type %d", it->ptype);
					value->set_success(false);
					break;
				}
			}
		}
